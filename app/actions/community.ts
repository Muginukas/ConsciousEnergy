...
export async function postComment(
  ...
  slug: string,
  body: string
): Promise<{ error: 'empty' | 'not_signed_in' | 'failed' } | { comment: Comment }> {
  ...
  if (error || !data) return { error: 'failed' as const }
...
export async function setRating(category: CategorySlug, slug: string, stars: number) {
  if (stars < 1 || stars > 5) return { error: 'invalid' as const }
